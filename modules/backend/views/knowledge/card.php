...
<?php
  ...
  $download_main = "<a href=".\yii\helpers\Url::to(['/backend/knowledge/download', 'type' => \app\models\FsFile::TYPE_MAIN, 'id' => $model->id], true)." target='_blank' class='download-link download-link-all'>Основной файл (". Yii::$app->formatter->asShortSize($model->getFsMainSize(), 0).")</a>";
  $download_additional = $model->fsFilesAdditional?"<a href=".\yii\helpers\Url::to(['/backend/knowledge/download', 'type' => \app\models\FsFile::TYPE_ALL, 'id' => $model->id], true)."  target='_blank' class='download-link-big download-link-all'>Все файлы (". Yii::$app->formatter->asShortSize($model->size, 0).")</a>":'';
