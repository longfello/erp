...
<ul class="root-category-items">
    ...
    <?php if ($item->checkAccessRecursive(\app\models\Fs::ACCESS_VIEW)){ ?>
      <li>
        <a class="ajaxify" href="<?= \yii\helpers\Url::to(['/backend/fileshare/index', 'root' => $item->id ]) ?>">
        ...
        <?php if ($item->checkAccess(\app\models\Fs::ACCESS_EDIT)){ ?>
          <ul class="functions">
            <li><a href="<?= \yii\helpers\Url::to(['/backend/fileshare/update', 'id'=>$item->id]) ?>" title="Редактировать"><i class="fa fa-pencil"></i></a></li>
            <li><a data-pjax="0" data-method="post" data-confirm="Удалить папку вместе со всем содержимым？" aria-label="Удалить" title="Удалить" href="<?= \yii\helpers\Url::to(['/backend/fileshare/delete', 'id'=>$item->id]) ?>"><i class="fa fa-remove"></i></a></li>
          </ul>
        <?php } ?>
      </li>
    <?php } ?>
